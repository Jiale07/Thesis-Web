<template>
  <div>
    <el-button type="text" @click="showDialog">更新公告</el-button>
...
<script>
import {getACList} from "../../../../../axios/adminView/Announcement/AnnouncementUpdate";
import {mapState} from "vuex";
import {postUpdateAnnouncementInfo} from "../../../../../axios/adminView/Announcement/AnnouncementDetail";
import {getRoleList} from "../../../../../axios/public/RoleAbout";

export default {
  name: "UpdateAnnouncementComponent",
  props:['Announcement','AVisibleList','ACategory'],
  data(){
  ...
  methods:{
    ...
    showDialog(){
      ...
      this.form.creatorId = this.Announcement.creatorId
      this.form.announcementName = this.Announcement.announcementName
      this.form.announcementContent = this.Announcement.announcementContent
      this.form.categoryId = this.ACategory.id
      let AVisibleList = this.AVisibleList
      let roleList = []
      AVisibleList.forEach((element,index)=>{
        roleList[index]=element.id
      })
      this.form2.roleList = roleList
      ...
      this.$refs[fromName].validate((valid)=>{
        if (valid){
          ...
          if (this.active ===2){
            ...
            let newRoleList = []
            roleList.forEach(e=>{
              newRoleList.push(this.roleFilter(e))
            })
            this.preview.roleList = newRoleList
    ...
    submitFrom(){
      ...
      postUpdateAnnouncementInfo(formJSON).then(result=>{
        let res = result.data
